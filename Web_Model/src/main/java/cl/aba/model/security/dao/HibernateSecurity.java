...
@Transactional(value = "transactionManagerSecurity")
public class HibernateSecurity extends HibernateDaoSupport {
protected Logger log = Logger.getLogger(getClass().getName());
	
	@Autowired
	public void annyMethodName(@Qualifier("sessionFactorySecurity") SessionFactory sessionFactory) {
	...
	public Object getObjectSql(String sql){
		...
		try{
			session = getHibernateTemplate().getSessionFactory().openSession();
			ret = session.createQuery(sql).uniqueResult();
		}catch (Exception e) {
	...
	public List<Object> getObjectListSqlTrans(Object object, String sql){
		List<Object> list = null;
		Session session = null;
		if (object != null) {
			try {
				...
				error("Al intentar realizar la consulta " + sql + " " + e.getMessage() + " Cause " + e.getCause());
			} finally {
				session.close();
			}

		} else {
			error("Objeto nulo, no se ha realizado la consulta -> " + sql);
		}
